...
*/
int mmcsCommandStatusIndex = ios_base::xalloc();

/*!
** ios_base::xalloc() returns a unique index associated with the ostream
** that we can use with ios_base::pword() to keep track of the MMCSCommandReply object associated with the stream
*/
int mmcsCommandReplyIndex = ios_base::xalloc();


/*!
** Default constructor - replies are written to stdout, format 0, write disabled
*/
MMCSCommandReply::MMCSCommandReply(unsigned fd, int replyFormat, bool enableWrite)
    : streambuf(),
      ...
      _outbuf(NULL)
{
    ...
    assert(_outbuf);
    ostream::pword(mmcsCommandReplyIndex) = this;
    reset();
}
...
*/
MMCSCommandReply::MMCSCommandReply(MMCSConsolePort* consolePort, int replyFormat, bool enableWrite)
    : streambuf(),
      ostream((streambuf*)this),
...
** Base destructor
*/
MMCSCommandReply::~MMCSCommandReply()
{
    ...
    ostream::pword(mmcsCommandReplyIndex) = NULL;
}
...
*/
bool
MMCSCommandReply::enableWrite(bool enable)
{
...
*/
void
MMCSCommandReply::reset()
{
    setp(_outbuf, _outbuf + _bufsize);
    ostream::iword(mmcsCommandStatusIndex) = STATUS_NOT_SET;
    _done = false;
    _replylen = 0;
    _totreplylen = 0;
}
...
** an MMCSCommandReply object from data received over a TCP/IP
** connection.
*/
void
MMCSCommandReply::assign(string& s)
{
    reset();
	...
	{
	    throw logic_error("invalid use of MMCSCommandReply::assign()");
	}
    }
    else
    {
...
*/
void
MMCSCommandReply::append(string& s)
{
    if (getStatus() == MMCSCommandReply::STATUS_NOT_SET || _done)
    {
	throw logic_error("invalid use of MMCSCommandReply::append()");
    }
    if (!s.empty())
    {
	// 'normalize' string to replyformat 1
...
*/
void
MMCSCommandReply::parseString(string& s, unsigned startPos)
{
    string::size_type endPos1, startPos2, endPos2;
    endPos1 = s.find(ABORT_TOKEN, startPos);
    endPos2 = s.find(DONE_TOKEN, startPos);

    if (endPos1 != string::npos)
	if (endPos2 != string::npos) // found ABORT & DONE
	{
    ...
    else if (endPos2 != string::npos) // found DONE but not ABORT
    {
...
/*!
** str() resturns the contents of the stream buffer as a standard string
*/
string
MMCSCommandReply::str(bool with_status, unsigned replyFormat)
{
    ...
    string replystr(reply_start, reply_len);
...
*/
void
MMCSCommandReply::dump()
{
    const int DUMPSIZE = 10;
    char* my_pbase = pbase();
    char* my_pptr  = pptr();
    char* my_epptr = epptr();
    char* last_data_ptr;
    char first_data[DUMPSIZE+1];
    char last_data[DUMPSIZE+1];
    strncpy(first_data, my_pbase, DUMPSIZE);
    first_data[DUMPSIZE] = 0;
    last_data_ptr = my_pptr-DUMPSIZE;
    if (last_data_ptr < my_pbase)
	last_data_ptr = my_pbase;
    strncpy(last_data, last_data_ptr, DUMPSIZE);
    last_data[DUMPSIZE] = 0;

    for(int i = 0; i < DUMPSIZE; ++i)
    {
	if (first_data[i] == '\n') first_data[i] = ';';
	if (first_data[i] == DONE_TOKEN) first_data[i] = '@';
	if (last_data[i] == '\n') last_data[i] = ';';
	if (last_data[i] == DONE_TOKEN)  last_data[i] = '@';
    }

    long int len = my_pptr - my_pbase;
    printf("status=%d _done=%d _totreplylen=%d iosState=0x%x _outbuf=%p pbase=%p pptr=%p epptr=%p len=%ld first_data=%.*s last_data=%.*s\n" \
           , getStatus(), _done, _totreplylen, rdstate(), _outbuf,my_pbase, my_pptr, my_epptr, len , DUMPSIZE, first_data, DUMPSIZE, last_data);
}
...
*/
int
MMCSCommandReply::overflow (int c)
{
...
// and write it to the MMCSConsolePort or stdout
//
...
MMCSCommandReply::send_buf()
{
    ...
    int i;			    // loop index
    bool found_null = false;        // true if this buffer contains terminating null (note: this is not the same meaning as MMCSCommandReply::_done)
    if (len > 0)
    {
	...
	else
	{
	    ...
	    if (*(_outbuf+len-2) == DONE_TOKEN) {
                //                std::cout << "reducing reply length!" << std::endl;
		...
		{
		    ostringstream msg;
		    msg << "MMCSCommandReply::send_buf() - " << strerror(errno);
		    LOG_ERROR_MSG(msg.str());
		    setp(_outbuf, _outbuf + _bufsize); // reset buffer pointers
		    *this << ABORT << msg.str() << DONE;
		    return -1;
...
*/
void
MMCSCommandReply::setStatus(int status)
{
...
**
** The first output to an MMCSCommandReply stream should always be OK or FAIL.
...
OK(ostream& stream)
{
    MMCSCommandReply* reply = (MMCSCommandReply*) stream.pword(mmcsCommandReplyIndex); // get the MMCSCommandReply for this stream
    assert(reply != NULL);
...
FAIL(ostream& stream)
{
    ...
    reply->reset();
    stream.iword(mmcsCommandStatusIndex) = MMCSCommandReply::STATUS_FAIL; // save the MMCS command status
    stream  << FAIL_TOKEN;
    return stream;
}

ostream&
ABORT(ostream& stream)
{
    MMCSCommandReply* reply = (MMCSCommandReply*) stream.pword(mmcsCommandReplyIndex); // get the MMCSCommandReply for this stream
    assert(reply != NULL);
    ...
    if (stream.iword(mmcsCommandStatusIndex) == MMCSCommandReply::STATUS_NOT_SET)
	return FAIL(stream);

    stream.iword(mmcsCommandStatusIndex) = MMCSCommandReply::STATUS_ABORT; // save the MMCS command status
    stream << ABORT_TOKEN;
    return stream;
}
...
DONE(ostream& stream)
{
    ...
    if (!reply->_done)
    {
	assert(stream.iword(mmcsCommandStatusIndex) != MMCSCommandReply::STATUS_NOT_SET);
	reply->_done = true;
	stream << DONE_TOKEN;
    }
    return stream;
