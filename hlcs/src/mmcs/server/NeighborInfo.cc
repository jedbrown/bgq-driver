        ...
        )
{
    ...
    if ( !strcmp(direction, "A+") ) {
    ...
    } else if ( !strcmp(direction, "D+") && _location.getType() == bgq::util::Location::ComputeCardOnNodeBoard ) {
        const std::string fromPos(nodeInfo->midplanePos());
        ...
        this->ioLink( rawdata );
        return;
    } else if ( direction[0] == 'D' && _location.getType() == bgq::util::Location::ComputeCardOnIoBoard ) {
        this->ioLink( rawdata );
        return;
    }
