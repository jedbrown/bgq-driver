...
void
BlockControllerBase::create_nodeset(
        std::vector<BCNodeInfo*> args,
        mmcs_client::CommandReply& reply
        )
{
    LOG_DEBUG_MSG(__FUNCTION__);
    if(args.size() < 1) {
        reply << mmcs_client::FAIL << "No nodes specified" << mmcs_client::DONE;
        return;
    }
    _delmachine = false;
    _nodeset = true;
        ...
        )
{
    ...
    LOG_DEBUG_MSG("target set " << _blockName << " created");
    ...
    if (mcOpenTargetSetReply._rc)
    {
        ...
        reply << mmcs_client::FAIL << "openTargetSet: " << mcOpenTargetSetReply._rt << mmcs_client::DONE;
        deque<string> dq;
        mmcs_client::CommandReply bogus;
        disconnect(dq, bogus);
        return 0;
        ...
        LOG_INFO_MSG("target set " << _blockName << " opened");
        ...
        )
{
    ...
    if (szOutFile.length() > 0)
    {
        ...
        if (pFile == NULL)
        {
            reply << mmcs_client::FAIL << "can't open file: " << szOutFile << " - " << strerror(errno) << mmcs_client::DONE;
            return;
        }
        ...
        )
{
    ...
    if (_mcServer == 0) {
        reply << "block is already disconnected" << mmcs_client::DONE;
        _disconnecting = false;
        return;
    }
    ...
    try
    {
        ...
        if (mcCloseTargetSetReply._rc) {
            ...
            LOG_INFO_MSG("target set " << _blockName << " closed");
        }
        ...
        if (mcDeleteTargetSetReply._rc) {
            ...
            LOG_INFO_MSG("target set " << _blockName << " deleted");
        }
    }
    catch (const std::exception &e)
    {
        ...
        )
{
    ...
    try {
        ...
        bgq::utility::PortConfiguration::parsePortsStr(ip + ":" + port, CLIENT_PORT_STR, portpairs);
    ...
    {
        reply << mmcs_client::FAIL << __FUNCTION__ << ": " << mcConnectReply._rt << mmcs_client::DONE;
        return;
    }
    reply << mmcs_client::OK << mmcs_client::DONE;
}
...
BCNodecardInfo *
BlockControllerBase::findNodecardInfo(
        string location
        )
{
...
void
BlockControllerBase::quiesceMailbox(
        const BlockControllerTarget* target
        )
{
    LOG_DEBUG_MSG(__FUNCTION__);
    int delay_timeout = 90; // default if not found in properties
    try {
        delay_timeout = boost::lexical_cast<int>( Properties::getProperty(SHUTDOWN_TIMEOUT) );
        if ( delay_timeout <= 0 ) {
            LOG_DEBUG_MSG( SHUTDOWN_TIMEOUT << " value must be > 0");
            delay_timeout = 90;
        }
    } catch ( const boost::bad_lexical_cast& e ) {
        LOG_DEBUG_MSG( "garbage " << SHUTDOWN_TIMEOUT << " value: " << e.what() );
    ...
    {
        LOG_INFO_MSG(__FUNCTION__ << "() waiting for ras events and node shutdown");
        time_t now;
        time(&now);
        if (_last_ras_event_time)       // wait for RAS events to slow down
        {
            const double secs_since_last_ras_event = difftime(now, _last_ras_event_time);
        ...
        if  (!_last_ras_event_time && !_shutdown_sent_time) break;
    ...
    if (_last_ras_event_time)
        LOG_WARN_MSG(__FUNCTION__ << "() discarding further RAS events");
    if (_shutdown_sent_time) {
        LOG_WARN_MSG(__FUNCTION__ << "() node shutdown did not complete from " << failed_nodes.size() << " nodes" );
        std::ostringstream logmsg;
        logmsg << "Failed nodes are ";
        for(std::vector<std::string>::const_iterator it = failed_nodes.begin(); it != failed_nodes.end(); ++it) {
            logmsg << *it;
        }
        ...
        )
{
    LOG_DEBUG_MSG(__FUNCTION__);
    if(!_redirectSock) return;
    if(isIOBlock() && _targetsetMode == MCServerMessageSpec::RAAW ) {
...
BlockControllerBase::checkShutdownComplete(
        const BlockControllerTarget* target
        )
{
    LOG_DEBUG_MSG(__FUNCTION__);
    ...
    if(target) {
        ...
        BOOST_FOREACH(const BCNodeInfo* node, target->getNodes()) {
            shutreq._location.push_back(node->_location);
        }
    } else {
        for(unsigned int i = 0; i < getNodes().size(); ++i) {
            const BCNodeInfo* ni = getNodes()[i];
            if(ni->_linkio == false)
    ...
    if(shutrep._compute_not_halted.empty()) {
        ...
        )
{
    ...
    const unsigned bad_locs = bootrep._compute_not_ready.size();
    LOG_DEBUG_MSG( bad_locs << " nodes failed to complete boot processing" );
