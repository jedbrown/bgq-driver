...
void
Properties::init(const std::string& filename, object mmcs)
{
    ...
    try {
        Properties::read( mmcs );
    } catch(Properties::PropertiesException& e) {
...
int Properties::getSubnets(std::string& hwstring) {
    int i = 0;
    try {
...
void Properties::getSubnetNames(std::vector<std::string>& subnet_names) {
    int i = 0;
    try {
        while(true) {  // Exception will end this loop.
            std::string subnet = "machinecontroller.subnet.";
...
void
Properties::read(object mmcs, bool bail)
{
    ...
    if(mmcs == server || mmcs == console) {
        ...
        for(bgq::utility::Properties::Section::iterator it = runjob_server_section.begin();
            ...
            if(it->first == "command_listen_ports") {
                ...
                Properties::setProperty("RUNJOB_PORTS", clp);
            }
        }
        if(!found)  {
            ...
            if(bail) throw PropertiesException(failmsg.str());
        }
    }
    ...
    if(mmcs == console) {
        ...
        try {
            ...
            if(bail) {
                ...
                throw PropertiesException(e.what());
            }
        ...
        for(bgq::utility::Properties::Section::iterator it = mmcs_console_commands.begin();
            ...
            Properties::addExternal(key, value);
        }
    ...
    for(std::vector<bgq::utility::Properties::Section>::iterator section_it = sections.begin();
        ...
        for(bgq::utility::Properties::Section::iterator it = (*section_it).begin();
            ...
            Properties::setProperty(key.c_str(), value);
    ...
    if(mmcs == server && Properties::getProperty(BRINGUP) == "true" && Properties::getProperty(RECONNECT_BLOCKS) == "true") {
        throw PropertiesException("Cannot reconnect to blocks if bringup=true in [machinecontroller] section");
    }
    if(mmcs == server && Properties::getProperty(BRINGUP) == "false" && Properties::getProperty(RECONNECT_BLOCKS) == "false") {
        throw PropertiesException("Illegal configuration.  [machinecontroller] bringup and [mmcs] reconnect_blocks cannot both be false.");
    }
...
Properties::Map* Properties::getExternalCmds() {
