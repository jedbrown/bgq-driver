...
#include "bgq_util/include/Regexp.h"
#include "MMCSCommandProcessor.h"

using namespace std;

BlockControllerTarget::BlockControllerTarget(BlockPtr blockController, const string& spec, MMCSCommandReply& reply)
...
//
bool
BlockControllerTarget::parseSpec(MMCSCommandReply& reply)
{
    if (_spec == "{*}")
    {
...
//
bool
BlockControllerTarget::parseRegexp(MMCSCommandReply& reply)
{
    const char *delim;
...
//
bool
BlockControllerTarget::parseGroup(MMCSCommandReply& reply)
{
    unsigned n, m;
...
//
bool
BlockControllerTarget::parseIndex(unsigned *index, MMCSCommandReply& reply)
{
    if (!isdigit(*_cur))
...
//
bool
BlockControllerTarget::error(const string& message, MMCSCommandReply& reply)
{
    reply << FAIL << "invalid target " << _spec << " : " << message << " near column " << _cur - _spec.c_str() + 1 << DONE;
    _nodes.clear();
    return false;
}
