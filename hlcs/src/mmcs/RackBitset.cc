...
void parse_location(std::string& location, unsigned& midplane, unsigned& board, unsigned& node) {
    // Location format of R00-M0-N00-J00
    // or Q00-I0-J00 or R00-I0-J00
    typedef boost::tokenizer<boost::char_separator<char> > tokenizer;
    boost::char_separator<char> sep("-");
    tokenizer tok(location, sep);
    ...
    if(location.length() > 10) {
        ...
        midplane = 0;  // IO always gets a zero midplane.
    }
    curr_string = *(++curr_tok);
...
int RackBitset::encode(std::string& location) {
...
void RackBitset::decode(unsigned bit, unsigned& midplane, unsigned& board, unsigned& node) {
...
void RackBitset::FindUnset(std::vector<std::string>& locations) {
...
std::string RackBitset::find_location(unsigned bit) {
    std::string location;
    unsigned midplane, board, node;
    midplane = board = node = 0;
...
void RackBitsetBase::Set(std::string& location) {
    if(!_IO) {  // Compute
        if(location.length() == 3 && location.find("R") != std::string::npos) {
            ...
            location += "-N00-J00";
            unsigned base_index = encode(location);
...
void RackBitsetBase::Reset(std::string& location) {
    unsigned index = encode(location);
    _nodes.set(index, false);
}

bool RackBitsetBase::Flagged(std::string& location) {
    unsigned index = encode(location);
    return _nodes.test(index);
}

void IOBoardBitset::FindUnset(std::vector<std::string>& locations) {
...
std::string IOBoardBitset::find_location(unsigned bit) {
    unsigned midplane, board, node;
    midplane = board = node = 0;
    decode(bit, midplane, board, node);
    std::ostringstream loc;
...
int IOBoardBitset::encode(std::string& location) {
    unsigned midplane, board, node;
    midplane = board = node = 0;
    parse_location(location, midplane, board, node);
...
void IOBoardBitset::decode(unsigned bit, unsigned& midplane, unsigned& board, unsigned& node) {
