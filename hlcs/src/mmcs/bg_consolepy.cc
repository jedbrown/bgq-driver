...
*/
class PyConsoleCommandReply : public MMCSCommandReply
{
public:
    PyConsoleCommandReply() :
        MMCSCommandReply(1, 0, false) {};
...
protected:
    void connect();
    boost::scoped_ptr<MMCSConsoleCommandProcessor> _commandProcessor;
    boost::scoped_ptr<ConsoleController> _pController;
    boost::scoped_ptr<ConsoleOptions> _options;
...
BgConsole::BgConsole(const string args)
{
    ...
    _options.reset(new ConsoleOptions( argc, &argv[0] ));
    ...
    _commandProcessor.reset( new MMCSConsoleCommandProcessor(mmcsCommands) );
    ...
    if(redirect_fd != "redirect_fd") // If we've got a redirect file descriptor, use it.
        _pController->setFd(boost::lexical_cast<int>(redirect_fd));
...
void BgConsole::connect()
{
    // connect to the mmcs server
    {
        ...
        deque<string> mmcs_connect = MMCSCommandProcessor::parseCommand("mmcs_server_connect " + _options->getRetry());
        PyConsoleCommandReply reply;
        //MMCSCommandReply reply(1, 0 /*_pController->getReplyFormat()*/, false);
