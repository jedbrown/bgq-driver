...
main(int argc, char *argv[])
{
    ...
    ConsoleController* console= NULL;                       // console interface to end user

    try {
        // parse options
        lite::Options options( argc, argv );

        // check sqlite thread safety
        LOG_DEBUG_MSG( "sqlite thread safety: " << ( sqlite3_threadsafe() ? "yes" : "no" ) );

        // ensure sqlite key is available
        if ( MMCSProperties::getProperty("sqlite").empty() ) {
        ...
        os << " clockHz='" << MMCSProperties::getProperty("clockHz")  << "' ";
        os << " bgsysIpv4Address='" << MMCSProperties::getProperty("nfsIpAddress")  << "' ";
        os << " bgsysRemotePath='" << MMCSProperties::getProperty("nfsExportDir")  << "' ";
        os << " serviceNodeIpv4Address='" << MMCSProperties::getProperty("serviceNode")  << "' ";
        os << " computeNodeMemory='" << MMCSProperties::getProperty("computeNodeMemory")  << "' ";

        if (MMCSProperties::getProperty("computeNodeMemory") == "4096")
        {
        ...
        os << " ioNodeMemory='" << MMCSProperties::getProperty("ioNodeMemory")  << "' ";
        if (MMCSProperties::getProperty("ioNodeMemory") == "4096")
        {
        ...
        os << " mtu='" << MMCSProperties::getProperty("mtu")  << "' ";
        os << " gateway='" << MMCSProperties::getProperty("gateway")  << "' ";
        os << " broadcast='" << MMCSProperties::getProperty("broadcast")  << "' ";
        os << " ipv4Netmask='" << MMCSProperties::getProperty("netmask")  << "' ";
        os << "  >";

        int rows = atoi(MMCSProperties::getProperty("rackRows").c_str());
        int cols = atoi(MMCSProperties::getProperty("rackColumns").c_str());
        char rowChar[2], colChar[2];
        char ncChar[3];
        int ncNum;

        struct in_addr ip;
        ip.s_addr = inet_addr(MMCSProperties::getProperty("ionodeip").c_str());
        int mac[6];
        //char macAddr[18];
        if (6 != sscanf(MMCSProperties::getProperty("ionodemac").c_str(), "%x:%x:%x:%x:%x:%x", &mac[0],&mac[1],&mac[2],&mac[3],&mac[4],&mac[5])) {
        ...
        if ( !MMCSProperties::getProperty("ioDrawer").empty() ) {
            os << " <BGQIODrawer";
            os << " posInMachine='" << MMCSProperties::getProperty("ioDrawer") << "'>" << endl;
            for (int ionode = 0;ionode < 8; ionode++) {

                //                    macInc(mac);
                //                    os << "<BGQIONode location='" << MMCSProperties::getProperty("ioDrawer") << "-J0" << ionode << "' ip='" << inet_ntoa(ip) << "' mac='" << string(macOut(mac,macAddr)) << "' />" << endl;
                os << "<BGQIONode location='" << MMCSProperties::getProperty("ioDrawer") << "-J0" << ionode << "' >" << endl;
                os << "<BGQNetConfig interface='external1' ";
                if ( MMCSProperties::getProperty("ioInterfaceName").empty() ) {
                    os << " name='ib0' ";
                    LOG_WARN_MSG( "missing ioInterfaceName key in mmcs_lite section of properties file, using ib0 as default" );
                } else {
                    os << " name='" << MMCSProperties::getProperty("ioInterfaceName") << "' ";
                }
        ...
        MMCSCommandAttributes attr;
        attr.mmcsLiteCommand(true);
        MMCSCommandAttributes mask;
        mask.mmcsLiteCommand(true);
        boost::scoped_ptr<MMCSCommandMap> mmcsCommands( MMCSCommandProcessor::createCommandMap(attr, mask) );
        ...
        readHistoryFile();
        console = new LiteConsoleController( commandProcessor, machine, uid );

        MMCSProperties::setProperty(HELP_OPTIONS, "all"); // show internal and exteral commands in help
        MMCSCommandReply reply;

        if (options.getDefaultListener()) {
            // create a default listener
            LiteControlEventListener* liteListener = LiteControlEventListener::getLiteControlEventListener();
        ...
        )
{
    ...
    if ( MMCSProperties::getProperty("ioLink").empty() ) {
        ...
        try {
            const bgq::utility::Properties::ConstPtr properties = MMCSProperties::getProperties();
    ...
    nodes.tokenize(MMCSProperties::getProperty("ioLink"),",");
    if (nodes.size() == 4) {
        os <<
