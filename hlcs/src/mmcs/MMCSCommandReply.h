...
#ifndef _MMCSCommandReply_H
#define _MMCSCommandReply_H
...
**
*/
class MMCSCommandReply : protected std::streambuf, public std::ostream, private boost::noncopyable
{
public:
    /*!
    ** Reply status values
    */
    ...
    /*!
    ** Base constructor
    ** @param consolePort	MMCSConsolePort if replies are written to a socket.
    ...
    */
    explicit MMCSCommandReply(MMCSConsolePort* consolePort, int replyFormat=0, bool enableWrite=true);

    /*!
    ** Default constructor - replies are written to stdout, format 0, write disabled
    */
    MMCSCommandReply(unsigned fd=1, int replyFormat=0, bool enableWrite=false);


    /*!
    ** Base destructor
    */
    virtual ~MMCSCommandReply();
...
private:
    MMCSConsolePort* _consolePort;      // console port to write to
