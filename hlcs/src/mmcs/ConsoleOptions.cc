    ...
    _loggingOptions( "ibm.mmcs" )
{
    ...
    _options.add_options()
        ("host", boost::program_options::value(&host), "host:port pairs for connecting to mmcs_server, defaults to 127.0.0.1:32031")
        ("waitforserver,w", boost::program_options::bool_switch(), "wait for mmcs_server if it's not already started")
    ...
    hidden.add_options()
        ...
        ("redirect_fd,f", boost::program_options::value(&fd), "pass in a file descriptor for redirect output")
        ("insecure,i", boost::program_options::bool_switch() )
        ;
    ...
    MMCSExternalCommand::setPropfile(p);

    MMCSProperties::init(_propertiesOptions.getFilename(), MMCSProperties::console);
    ...
    // check for insecure
    if ( _vm["insecure"].as<bool>() ) {
        MMCSProperties::setProperty(SECURE_CONSOLE, "false");
    }

    if(_vm["no_shell"].as<bool>()) {
        MMCSProperties::setProperty(NO_SHELL, "true");
    } else MMCSProperties::setProperty(NO_SHELL, "false");

    if(!reconnect.empty()) {
        if(reconnect == "true" || reconnect == "false")
            MMCSProperties::setProperty(AUTO_RECONNECT, reconnect.c_str());
        else {
        ...
        MMCSProperties::setProperty(AUTO_RECONNECT, "true");
    }

    if(fd != "") {
        MMCSProperties::setProperty(REDIRECT_FD, fd.c_str());
    } else MMCSProperties::setProperty(REDIRECT_FD, "redirect_fd");
...
ConsoleOptions::openProperties()
{
    ...
    try {
        ...
        // eat this exception, we'll flag it later when opening MMCSProperties
    }
}

void
ConsoleOptions::setupLoggingDefaults()
{
