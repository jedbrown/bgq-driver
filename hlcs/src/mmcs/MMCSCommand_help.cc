...
// If there's no auth, we're done.
bool parseCommand(std::string& command, std::string& name, std::string& description, std::string& authlist) {
    ...
    tokenizer::iterator tokit = bar_tok.begin();
    name = *tokit;
    ++tokit;
    ...
    tokenizer::iterator ptokit = dollar_tok.begin();
    description = *ptokit;
    if(description.at(0) == ' ')
...
std::string stringify_auths(MMCSCommand* cmd) {
    std::ostringstream authstringstream;
    std::vector<MMCSCommandAttributes::AuthPair>* auths = cmd->attributes().getAuthPairs();
    bool first = true;
    for(std::vector<MMCSCommandAttributes::AuthPair>::iterator it = auths->begin();
        it != auths->end(); ++it) {
        if(!first)
            authstringstream << ",";
...
** Print command summary or extended help
** options:
**
**     all		Include both external and internal use commands in command summary
**     <command-name>	Print extended help for specified command
*/
MMCSCommand_help*
MMCSCommand_help::build()
{
    ...
    return new MMCSCommand_help("help", "help [<command-name>|<category>] [ auth ]", commandAttributes);
}

void
MMCSCommand_help::execute(deque<string> args,
			  MMCSCommandReply& reply,
			  ConsoleController* pController,
			  BlockControllerTarget* pTarget)
{
    MMCSCommandMap*  commandMap = pController->getCommandProcessor()->getCommandMap();
    MMCSCommandProcessor mmcsCommandProcessor(commandMap);
    mmcsCommandProcessor.logFailures(false);
    MMCSCommandReply serverReply;
    map<string, string> helpMap;
    unsigned cmdname_width = 23;
   ...
   if ((args.size() == 0) ||
       ...
       if (args.size() >= 1 && args[0] == "target") {
           ...
           for (MMCSCommandMap::iterator pos = commandMap->begin(); pos != commandMap->end(); ++pos)
               {
                   MMCSCommand* cmd = pos->second;
           ...
           for(map<string,string>::iterator pos = clientMap.begin(); pos != clientMap.end(); ++pos) {
               std::string name, description, authlist;
               name = pos->first;
           ...
           reply << OK;
           for (map<string,string>::iterator pos = helpMap.begin(); pos != helpMap.end(); ++pos)
               {
   ...
   if(args[0] == "auth") {
       reply << FAIL << "\"auth\" must be used with a category specified.\nType \"help help\" for more information." << DONE;
       return;
   }

   MMCSCommandMap::iterator pos = commandMap->find(args[0]);
   if (pos != commandMap->end()) // found the command in our commandMap
       {
           // execute the command's help method
           MMCSCommand* cmd = pos->second;
           MMCSCommandReply cmdReply;
           ...
           if(cmd->attributes().externalCommand() == false) {
               reply << OK << cmdReply.str() << "\nAuthorizations required are: "
                     << authstring << DONE;
           } else {
               reply << OK << cmdReply.str() << DONE;
           }
       }
   else		// didn't find the command in our command map
       {
           ...
           mmcsCommandProcessor.execute("mmcs_server_cmd",cmdStr, serverReply, pController);
           if (serverReply.getStatus() == 0)
               reply << OK << serverReply.str() << DONE;
           else
               reply << FAIL << "no help available for command or category " << args[0] << DONE;
           return;
       }
}

void
MMCSCommand_help::help(deque<string> args,
		       MMCSCommandReply& reply)
{
