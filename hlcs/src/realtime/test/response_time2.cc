...
{
   SharedState *shared_state_p(static_cast<SharedState*>(arg_void));
   ...
   cerr << "Connecting to realtime server.\n";
...
void response_time2( int client_count )
{
   ...
   pthread_t *client_threads(new pthread_t[client_count]);
   for ( int i(0) ; i < client_count ; ++i ) {
      int create_rc(pthread_create( &(client_threads[i]), 0, &client_thread, &shared_state ));
      if ( create_rc != 0 ) {
   ...
   cout << "All the clients are connected!\n";
   ...
   for ( int i(0) ; i < client_count ; ++i ) {
      int join_rc(pthread_join( client_threads[i], 0 ));
      if ( join_rc != 0 ) {
