...
/*!
 * \page revoke_job_authority
 *
 * Remove job access control.
 *
 * \section SYNOPSIS
 *
 * revoke_job_authority id user action [OPTIONS]
 *
 * \section DESCRIPTION
 *
 ...
 *
 * The user ID from which to revoke the authority. This parameter may be specified as the second positional argument.
 *
...
namespace server {
namespace commands {
namespace revoke_job_authority {
...
{
    return
        "Revokes authority to perform <action> from <user> on job <id>.\n"
...
Options::doValidate() const
{
    ...
    request->_id = _job;
    request->_mode = runjob::commands::request::JobAcl::Revoke;
    request->_user = _user;
    request->_action = _action;
}

} // revoke_job_authority
} // commands
} // server
} // runjob
