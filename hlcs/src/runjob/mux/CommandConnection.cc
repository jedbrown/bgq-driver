...
CommandConnection::~CommandConnection()
{
    ...
    if ( mux ) {
        mux->getConnections()->remove( _socket );
    }
}
        ...
        )
{
    ...
    runjob::commands::Message::Ptr request;
    request = runjob::commands::convert( _incomingHeader, _incomingMessage );
    if ( !request) {
...
void
CommandConnection::resolveComplete() 
{
    // add ourself to container
    if ( Multiplexer::Ptr mux = _mux.lock() ) {
        mux->getConnections()->add( this->getShared() );
    }
