...
uint64_t sc_stat64(SYSCALL_FCN_ARGS)
{
  ...
  if (!VMM_IsAppAddress(buf, sizeof(struct stat64))) {
  ...
  // Run file system's stat64() method.
  return File_GetFSPtrFromPath(pathname)->stat64(pathname, buf);
}
