...
uint64_t sc_sched_get_priority_max(SYSCALL_FCN_ARGS)
{
    ...
    {
        return CNK_RC_SUCCESS(CONFIG_SCHED_MAX_FIFO_PRIORITY);
    }
    else if (sched_policy == SCHED_COMM)
    {
        return CNK_RC_SUCCESS(CONFIG_SCHED_MAX_COMM_PRIORITY);
    }
    else
    {
