...
int DDR_InitSequence(struct DDRINIT_metrics ddr, int shmoo)
{
	...
	unsigned	i=0, caltry=0, ddtry=0, sbtry=0, rr;
...
MC_DEBUG(("Step 6. Load IOM registers\n\n"));
	...
	DDR_Init_IOM(&ddr, MC_IOM, REPRO);

if(!ddr.FAST) {
/*----------------------------------------------------------------------------------------------------------------------------------------*/ 
MC_DEBUG(("Step 7. Adjust ADDR delay\n\n"));

	DDR_Init_ADDR_Delay(MC_IOM, ddr.ADDR_DELAY, ddr.TIS_SHIFT, REPRO);
}
...
MC_DEBUG(("Step 11A.Detect Ranks and Fine Write Leveling\n\n"));
	...
	for(i=0; i<4 ;i++)
	{
		DDR_Init_Detect_Ranks(&num_ranks, dram_density, &ddr, MC_IOM, ODT, REPRO);
		if(num_ranks !=0)
			break;
	...
	if(num_ranks==0)
	{
	    ...
	    fw_writeRASEvent( FW_RAS_DDR_INIT_WARNING, 1, details );
...
MC_DEBUG(("Step 12A.Master_Control_Error_Register check\n\n"));
	...
	if(DDR_Init_MasterCtrlErrReg(&ddr, MC_IOM, 0)!=0)
	{
		if(caltry < NUM_CAL_RETRY)
		{
			caltry++;
			if(caltry >= 2 && (ddr.IMPEDANCE & 0x300) != 0)
				ddr.IMPEDANCE -= 0x100;
			MC_DEBUG(("Retrying with Impedace = 0x%lx\n",ddr.IMPEDANCE));
...
MC_DEBUG(("Step 13. Coarse Write Leveling\n\n"));
...
#endif
	{
		...
		{
			DDR_Init_ADDR_Delay(MC_IOM, ADDR_DELAY%0x200, ddr.TIS_SHIFT, REPRO);
			DDR_Init_Reset_DRAM(&ddr);
			DDR_Init_MRS(&ddr);
			DDR_Init_FWL(num_ranks, &ddr, MC_IOM, ODT, REPRO);
			if(!ddr.SKIP_RDCAL)
	...
	if(local_pass==0)
	{
		...
		printf("ERROR: CWL failed. Forcing ADDR_DELAY to default value (%X)\n",ADDR_DELAY);	// 1-30-2012
	}
	else if(ADDR_DELAY!=ddr.ADDR_DELAY_START)
		printf("WARNING: CWL adjusted ADDR_DELAY to %X\n",ADDR_DELAY);		// 1-30-2012
#endif
    }
...
//	if(dram_density == 256)
	...
	{
		if(ddtry==0 && (ddr.IMPEDANCE & 0x300) != 0)
		{
			ddr.IMPEDANCE &= 0xFCFF;
			caltry=0;
			ddtry=1;
			MC_DEBUG(("Retrying with Impedace = 0x%lx\n",ddr.IMPEDANCE));
			goto STEP6;
...
if ( FW_IS_NODE_0 && sbtry==0 ) {
    printf("DDR3 %dMbps, %d%sb SDRAM, %d-Rank%s %dGByte, DD%d%s, PD_%s, BackScrub=%s DirScrub=%s DDR_FW_ver = %s\n", dram_speed, (dram_density>512)?dram_density/1024:dram_density, (dram_density>512)?"G":"M", num_ranks, (num_ranks==1)?",":"s,", 4*dram_density*num_ranks/1024, TI_isDD1()?1:2, FW_DD1_WORKAROUNDS_ENABLED()?", Firmware workarounds enabled":"", ddr.PD_DISABLE?"Off":(ddr.PD_FAST?"Fast":"Slow"), (ddr.BG_SCRUB_DISABLE?"Off":"On"),PERS_ENABLED(PERS_ENABLE_DDRDirScrub)?"On":"Off",DDR_FW_ver);
}
...
MC_DEBUG(("Step 17. Small Block Test\n\n"));
	...
	MC_DEBUG(("Writing...\n"));
	for(val=0; val<MAX_ROW_ADDR(dram_density)/4; val++)
		for(i=0; i<2 ;i++)
			for(rr=0; rr<num_ranks ;rr++)
				MEM_Alter_addr(i, rr, (val&0x1C00)>>10, val, val&0x3FF, CWL_wrdata_pat[val%3]);
	MC_DEBUG(("Reading...\n"));
	uint64_t FpatL=0, FpatH=0;
	for(local_pass=1,val=0; val<MAX_ROW_ADDR(dram_density)/4; val++)
		for(i=0; i<2 ;i++)
			for(rr=0; rr<num_ranks ;rr++)
			{
				MEM_Display_addr(i, rr, (val&0x1C00)>>10, val, val&0x3FF, rddata);
				local_pass &= Mem_Compare(CWL_wrdata_pat[val%3], rddata);
				FpatL |= Mem_Compare_8L(CWL_wrdata_pat[val%3], rddata);
				FpatH |= Mem_Compare_8H(CWL_wrdata_pat[val%3], rddata);
			}
	if(local_pass!=1)
	{
		if(sbtry==0 && (ddr.IMPEDANCE & 0x300) != 0)
		{
			ddr.IMPEDANCE &= 0xFCFF;
			caltry=0;
			ddtry=0;
		...
		details[1] = FpatL;
		details[2] = FpatH;
		fw_writeRASEvent( FW_RAS_DDR_INIT_WARNING, 3, details );
...
//	val |= _BN(29);			// disable hardware marking
	...
	for(i=0; i<2 ;i++)
	{
		...
		if(!smb_pass)
		{
			for(local_pass=1, rr=0; rr<num_ranks; rr++)
				if((j=ddr.MRKCHIP[i][rr]) < 18)
				{
					local_pass=0;
					if(ddr.MRKCHIP[i][(num_ranks-1) - rr] >= 18)
					{
					//	if(!TI_isDD1())
					//		Mem_Marking(i,rr,j);
					...
					//		(rr==0?"T":(rr==1?"B":(rr==2?"U":"D"))), i==0?"R":"L");
					    ...
					    details[1] = rr;
					    details[2] = j;
					    fw_writeRASEvent( FW_RAS_BAD_DRAM_WARNING, 3, details );
					}
					else
					{
						fw_uint64_t details[2];
						details[0] = i;
						details[1] = j;
						fw_writeRASEvent( FW_RAS_BAD_PHY_WARNING, 2, details );
