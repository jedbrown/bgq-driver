...
{
    if ( curl_easy_setopt( curl_handle.get(), CURLOPT_HEADERFUNCTION, &_handle_header ) != CURLE_OK ) {
        BOOST_THROW_EXCEPTION( runtime_error( string() + "failed to set the CURLOPT_HEADERFUNCTION option on the curl handle, " + curl_handle.getErrorBuffer() ) );
    }

    if ( curl_easy_setopt( curl_handle.get(), CURLOPT_WRITEHEADER, this ) != CURLE_OK ) {
        BOOST_THROW_EXCEPTION( runtime_error( string() + "failed to set the CURLOPT_HEADERFUNCTION option on the curl handle, " + curl_handle.getErrorBuffer() ) );
    }

    if ( curl_easy_setopt( curl_handle.get(), CURLOPT_WRITEFUNCTION, &_handle_data ) != CURLE_OK ) {
        BOOST_THROW_EXCEPTION( runtime_error( string() + "failed to set the WRITEFUNCTION option on the curl handle, " + curl_handle.getErrorBuffer() ) );
    }

    if ( curl_easy_setopt( curl_handle.get(), CURLOPT_WRITEDATA, this ) != CURLE_OK ) {
        BOOST_THROW_EXCEPTION( runtime_error( string() + "failed to set the WRITEDATA option on the curl handle, " + curl_handle.getErrorBuffer() ) );
    }
}


json::ValuePtr Response::getJsonResponse() const
{
    json::ValuePtr value_ptr(json::Parser()( _data ));
