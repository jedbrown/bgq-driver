...
#include "DiagnosticsLocations.hpp"
...
const string DiagnosticsLocations::HARDWARE_STATUS_STRING_TO_ORD_CLAUSE =
        "WHEN 'success' THEN 0"
...
const string DiagnosticsLocations::HARDWARE_STATUS_ORD_TO_STRING_CLAUSE =
        "WHEN 0 THEN 'success'"
...
DiagnosticsLocations::DiagnosticsLocations(
        const DiagnosticsLocationsOptions& options
    ) :
        _options(options)
{
    // Nothing to do.
...
void DiagnosticsLocations::execute(
...
uint64_t DiagnosticsLocations::_calcRowCount( cxxdb::ConnectionPtr conn_ptr )
{
    WhereClause loc_wc;
    WhereClause main_wc;
...
void DiagnosticsLocations::_doQuery(
        cxxdb::ConnectionPtr conn_ptr,
        cxxdb::ResultSetPtr* rs_ptr_out
    )
{
