...
void Node::_doGet()
{
    ...
    if ( all_count != 0 ) {
        while ( rs_ptr->fetch() ) {
            ...
            obj.set( "location", rs_ptr->columns()[BGQDB::DBTNodeenvironment::LOCATION_COL].getString() );
            obj.set( "time", rs_ptr->columns()[BGQDB::DBTNodeenvironment::TIME_COL].getTimestamp() );
            obj.set( "temp", rs_ptr->columns()[BGQDB::DBTNodeenvironment::ASICTEMP_COL].as<int32_t>() );
        }
    }
