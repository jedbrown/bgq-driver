...
/*! \page nodeEnvironmentalsResource /bg/environmentals/node

Node environmentals.

- \subpage nodeEnvironmentalsResourceGet

 */

/*! \page nodeEnvironmentalsResourceGet GET /bg/environmentals/node

Query node environmentals.
...
- asicTemp: \ref numberComparisonFormat "integer number comparison"
...
#include "../../query/env/NodeOptions.hpp"
...
const capena::http::uri::Path Node::RESOURCE_PATH(capena::http::uri::Path() / "environmentals" / "node");
...
void Node::_doGet()
{
    ...
    boost::shared_ptr<query::env::NodeOptions> options_ptr( new query::env::NodeOptions() );
    ...
    _query_ptr->executeAsync(
            ...
            _getStrand().wrap( boost::bind( &Node::_queryComplete, this,
    ...
    try {
        ...
        if ( res.all_count != 0 ) {
            while ( res.rs_ptr->fetch() ) {
                ...
                obj.set( "location", res.rs_ptr->columns()[BGQDB::DBTNodeenvironment::LOCATION_COL].getString() );
                obj.set( "time", res.rs_ptr->columns()[BGQDB::DBTNodeenvironment::TIME_COL].getTimestamp() );
                obj.set( "temp", res.rs_ptr->columns()[BGQDB::DBTNodeenvironment::ASICTEMP_COL].as<int32_t>() );
            }
        }
