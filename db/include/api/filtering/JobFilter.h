...
/*!
 * \file api/filtering/JobFilter.h
 * \brief JobFilter class definition.
 */

#ifndef BGQDB_FILTERING_JOBFILTER_H_
#define BGQDB_FILTERING_JOBFILTER_H_
...
public:
     ...
     */
    typedef std::set<job::status::Value> Statuses;
    ...
    /*!
     * \brief Block creation Id
     */
    typedef uint64_t BlockCreationId;
     ...
     */
    void setJobId(
            const Id jobId      //!< [in] Job id to filter on
            );
     ...
     * the BGQDB::filtering::getjobs() API. For example, when searching for "Active" jobs any of the statuses for completed
     * jobs are ignored. When searching for "Completed" jobs any of the statuses for active jobs are ignored. When searching
     * for "All" jobs both the active job table and the job history table are queried separately and the results combined.
    ...
    void setStartTimeInterval(
            const TimeInterval timeInterval  //!< [in] Start time interval to filter on
    );
     ...
     */
    const TimeInterval getStartTimeInterval() const;
    ...
    void setEndTimeInterval(
            const TimeInterval timeInterval  //!< [in] End time interval to filter on
    );
...
protected:
    ...
    std::string     _computeBlockName;
    std::string     _user;
    std::string     _executable;
    Statuses        _status;
    JobType::Value  _jobType;   //!< Job type (Active, Completed or All)
    bool            _isExitStatusSet;
    int             _exitStatus;
