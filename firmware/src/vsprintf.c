...
static char* fmt_U64(char *p, uint64_t v, unsigned flags, int prec, int width, int neg )
{
  int i, j, len;
  int pad = ((flags & FLAG_ZEROPAD) ? '0' : ' ');
  char buf[64];
...
static char* fmt_I64( char *p, uint64_t v, unsigned flags, int prec, int width )
{
   ...
   if ( ( v & 0x8000000000000000ul ) != 0 )
      {
      neg = 1;
      v = -v;
      }

   return fmt_U64( p, v, flags, prec, width, neg );
}
...
typedef char*(*FORMATTER)(char *p, uint64_t v, unsigned flags, int prec, int width);
    ...
    for (; *fp ; fp++ )
    {
        ...
        while(1)
        {
            ...
            if(ctrl && !(ctrl &  FLAG_FLAGS)) 
            {
                ...
                p = ((FORMATTER)(ctrl))(p, ull, flags, prec, width);   
                break;
            }
            else
            {
