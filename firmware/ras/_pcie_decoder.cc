...
void decode_Root_Complex_Status_Register( ostringstream& str, uint64_t status ) {
  ...
  const std::string pci_cfg_wr_error_retry_DESCR = " [PCI_CFG_WR_ERROR_RETRY] Outbound PCIe configuration or IO write completed with a retry error";
  ...
  const std::string       pci_mem_rd_error_DESCR = " [PCI_MEM_RD_ERROR] Outbound PCIe memory read completed in error";
  const std::string    pci_mem_rd_error_ur_DESCR = " [PCI_MEM_RD_ERROR_UR] Outbound PCIe memory read completed with an unsupported request error";
  const std::string pci_mem_rd_error_rerry_DESCR = " [PCI_MEM_RD_ERROR_RERRY] Outbound PCIe memory read completed with a retry error";
...
void decode_PCI_Express_Interrupt_State_Register( ostringstream& str, uint64_t status ) {
  ...
  if ( ( status & ((uint64_t)1 << (63-0-34) ) ) != 0 ) // pci_mem_rd_error
    str << pci_mem_rd_error_DESCR + ";";

  if ( ( status & ((uint64_t)1 << (63-0-35) ) ) != 0 ) // pci_mem_rd_error_ur
    str << pci_mem_rd_error_ur_DESCR + ";";

  if ( ( status & ((uint64_t)1 << (63-0-36) ) ) != 0 ) // pci_mem_rd_error_rerry
    str << pci_mem_rd_error_rerry_DESCR + ";";
}
