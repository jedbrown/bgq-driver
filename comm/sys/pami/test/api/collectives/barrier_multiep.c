...
static void * barrier_test(void* p)
{
  ...
  for(nalg = 0; nalg < total_alg; nalg++)
  {
    ...
    if(ep_task_id == 0)
    {
      printf("# Barrier Test(correctness) context = %d protocol: %s (%d of %d algorithms)\n",
             td->tid, gProtocolName, nalg + 1, total_alg);
      printf("# -------------------------------------------------------------------\n");
    ...
    int niter = gNiterlat;
    if((ep_task_id == 0) || (gVerbose>2))
    {
      printf("# Barrier Test(performance) context = %d protocol: %s (%d of %d algorithms)\n",
             td->tid, gProtocolName, nalg + 1, total_alg);
      printf("# -------------------------------------------------------------------\n");
    }
    blocking_coll(myContext, &newbarrier, &poll_flag);
