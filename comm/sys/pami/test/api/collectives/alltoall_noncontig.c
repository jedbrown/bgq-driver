...
void init_bufs(size_t l, size_t r)
{
  size_t k;
  size_t d = l * r * 2;
  double * sdbuf = (double *)sbuf;
  double * rdbuf = (double *)rbuf;
  for (k = 0; k < l*2; k++)
  {
    sdbuf[ d + k ]    = (((double)(r + k) * 1.0));
    rdbuf[ d + k ]    = 0xffffffffffffffff;
  }
}
...
/*  double * sdbuf = (double*)sbuf;*/
  for (r = 0; r < nranks; r++)
  {
    ...
    for (k = 0; k < l * 2; k+=2)
    {
      if (dbuf[ d + k + 1] != (double)(((myrank + k) * 1.0) ))
      {
...
/*  printf("dbuf[ %d ]: %f value: %f  sbuf[ %d ]: %f\n", d + k + 1, (double)dbuf[ d + k + 1], (double)(((double)(myrank + k) * 1.0)), d+k+1, sdbuf[ d + k ]);*/
    }
  }
  return 0;
}
  ...
  for (; iContext < gNum_contexts; ++iContext)
  {
    ...
    for (nalg = 0; nalg < total_alg; nalg++)
    {
      ...
      for (i = 0; i <= (gMax_byte_count/(sizeof(double)*2)); i *= 2)
      {
        ...
        alltoall.cmd.xfer_alltoall.stype         = pami_stype;
        alltoall.cmd.xfer_alltoall.stypecount    = i;
        alltoall.cmd.xfer_alltoall.rcvbuf        = rbuf;
        alltoall.cmd.xfer_alltoall.rtype         = pami_rtype;
