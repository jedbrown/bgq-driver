...
public:
	...
	// NOTE: currently, all PAMI_Context_post() funcs are OneShot,
	// so cannot act like persistent work here.
...
int main(int argc, char **argv) {
	...
	int ret = 0;
	QueueTest<3> test1("default", pthreads, elements, seed, contexts);
	ret = test1.run_test();

	rc = PAMI_Context_destroyv(contexts, QueueTest<>::num_ctx);
	assert(rc == PAMI_SUCCESS);
