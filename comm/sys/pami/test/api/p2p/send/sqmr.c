...
#define NMASTERS   2       /* The number of master processes */
#define NWORKERS   32      /* The maximum number of worker processes */
...
#define MAX_SIZE   (NMASTERS * (NWORKERS+1)) /* The number of processes that can participate */
...
static size_t nmasters, nworkers;
...
master()
{
  ...
  const size_t dest0 = rank*nworkers+nmasters;
  size_t dest;
  size_t iteration, i;
  pami_work_t work_list[WINDOW][size];
  TRACE_ERR("partners=[%zu..%zu]\n", dest0, dest0+nworkers-1);

  for (iteration=0; iteration<ITERATIONS; ++iteration) {
    TRACE_ERR("starting sends  iteration=%zu\n", iteration);
    for (i=0; i<WINDOW; ++i) {
      for (dest=dest0; dest<dest0+nworkers; ++dest) {
...
worker()
{
  ...
  const size_t dest0 = (rank-nmasters)/nworkers;
  const size_t dest = dest0;
  size_t iteration, i;
  pami_work_t work_list[WINDOW][1];
  TRACE_ERR("partner=[%zu]\n", dest0);
...
init()
{
  ...
  nmasters = MIN(NMASTERS, size>>1);
  nworkers = MIN(NWORKERS, (size-nmasters)/nmasters);
  size = nmasters * (nworkers+1);
