...
#At this time, we are not automatically adding the CPL
    ...
    # Try to remove C++ comments in C code.  This only changes comment
    # blocks not at the beginning of a line, since the doxygen \file
    # block cannot be easily changed by scripts.
    if ($ARGV =~ m/\.c$/) {
        s, ///(.*?) *$, /**$1 */,mg;
        s, //(.*?) *$, /*$1 */,mg;
    }
    ...
    if ( ($ARGV =~ m/\.h$/) && ($ARGV !~ m/\.x\.h$/) && ($ARGV !~ m,/fort_inc(64)?/,) && ($ARGV !~ m,pami_sys.h$,) ){
        my $exclude = $ARGV;
        $exclude =~ s@^\./@@g;
        $exclude =~ s@^sys/@@g;
