...
RdmaConnection::~RdmaConnection(void)
{
   ...
   if (_cmId != NULL) {
      ...
      if (rdma_destroy_id(_cmId) == 0) {
         ...
         int err = errno;
         LOG_ERROR_MSG(_tag << "error destroying rdma cm id " << _cmId << ": " << bgcios::errorString(err));
      }
   }
...
RdmaConnection::createId(void)
{
   ...
   if (err != 0) {
      ...
      LOG_ERROR_MSG(_tag << "error creating rdma cm id: " << bgcios::errorString(err));
      throw e;
   }
...
RdmaConnection::resolveAddress(struct sockaddr_in *localAddr, struct sockaddr_in *remoteAddr)
{
   ...
   if (rc != 0) {
      ...
      LOG_ERROR_MSG(_tag << "error resolving remote address " << addressToString(remoteAddr) << ": " << bgcios::errorString(e.errcode()));
      throw e;
   }
   ...
   err = ackEvent();
   if (err != 0) {
      LOG_ERROR_MSG(_tag << "error acking " << rdma_event_str(_event->event) << ": " << bgcios::errorString(err));
...
RdmaConnection::accept(void)
{
   ...
   if (err != 0) {
      err = abs(err);
      LOG_ERROR_MSG(_tag << "error accepting connection: " << bgcios::errorString(err));
      return err;
   }
...
RdmaConnection::resolveRoute(void)
{
   ...
   if (rc != 0) {
      int err = errno;
      LOG_ERROR_MSG(_tag << "error resolving route: " << bgcios::errorString(err));
      return err;
   }
...
RdmaConnection::disconnect(bool initiate)
{
   ...
   if (err != 0) {
      err = abs(err);
      LOG_ERROR_MSG(_tag << "error disconnect: " << bgcios::errorString(err));
      return err;
   }
...
RdmaConnection::postSendQ(struct ibv_send_wr *request)
{
   ...
   if (err != 0) {
      if (err==EINVAL)
      {
        ...
        LOG_ERROR_MSG(_tag << "error posting to send queue: " << bgcios::errorString(err));
...
RdmaConnection::ackEvent(void)
{
   ...
   if (err != 0) {
      err = abs(err);
      LOG_ERROR_MSG(_tag << "error acking event " << rdma_event_str(_event->event) << ": " << bgcios::errorString(err));
      return err;
   }
