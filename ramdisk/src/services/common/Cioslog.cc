...
void printMsg(uint32_t ID, bgcios::MessageHeader *mh){
   printf("%s: type=%u rank=%u jobid=%llu sequenceId=%d length=%d\n", CIOS_FLIGHTLOG_FMT[ID],(int)mh->type,(int)mh->rank,(LLUS)mh->jobId, (int)mh->sequenceId,(int)mh->length);
}
#undef LLUS 
...
uint32_t logMsgWC(uint32_t ID,bgcios::MessageHeader *mh,struct ibv_wc *wc){
   ...
   entry->data[2] = msg[2];
   entry->data[3] = msg[3];
   entry->ci.BGV_recv[0] = wc->src_qp; //remote QP number
...
size_t Flight_CIOS_MsgDecoder(size_t bufsize, char* buffer, const BG_FlightRecorderLog_t* log)
{   
    ...
    size_t length = (size_t)snprintf(buffer, bufsize, "%8x ",log->entry_num);
    buffer += length;
    bufsize -= length;

    length = (size_t)snprintf(buffer, bufsize, "%s: ",CIOS_FLIGHTLOG_FMT[log->id]);
    buffer += length;
    bufsize -= length;
    ...
    if (log->id ==BGV_SEND_MSG){
        length = (size_t)snprintf(buffer, bufsize, " lQP=%d ",log->ci.BGV_recv[0] );
        buffer += length;
        bufsize -= length;    
   }
