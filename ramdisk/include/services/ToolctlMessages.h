...
const uint8_t ProtocolVersion = 6; 
...
const uint16_t SendSignalToProcess      = 253;
const uint16_t SendSignalToProcessAck   = 254;
...
//! Watchpoint limit checking and behavior policy.

enum WatchPolicy
{
    Watch_Propogate = 0, //!< Cloned threads inherit watchpoints from their parent    
    Watch_NoPropagate,   //!< No inheritance of watchpoints for cloned threads 
};
...
//! Notification signal execution mode. 

enum NotifySignalExecuteMode
{
    NotifySignal_NonSpeculative,  //!< Thread was not running speculatively.
    NotifySignal_Speculative,     //!< Speculative Execution or Transactional Memory mode was active.
};
...
enum SpecAccess
{
    SpecAccess_UseThreadState = 0,  //!< Read and write operations will use the current thread's state (speculative/non-speculative).
...
enum CmdReturnCode
{
    ...
    CmdPrevCmdFail = 14,     //!< A previous command failure with the same message has prevented this command from being executed.
    CmdProcessExiting = 15,  //!< A process is exiting. The command cannot be completed.
    CmdWatchConflict = 16    //!< A request to set a watchpoint overlaps with an existing watchpoint
};
...
enum BG_Thread_State
{
    ...
    FLock        //!< Thread is waiting for a file lock.
};
...
enum WatchType
{
    WatchNone = 0,  //!< No watch is active.
...
public:
    ...
    WatchPolicy       watchpointPolicy; //!< Policy regarding control of watchpoints
...
struct NotifyMessage : public ToolMessage
{
    ...
    union
    {
        struct {                                //!< Definition for NotifyMessageType_Signal.
            ...
            NotifySignalExecuteMode executeMode;//!< Was speculative execution or transactional memory mode active.
            struct {
...
{
    uint32_t length;                    //!< Length of data in doublewords.
...
struct SendSignalToProcessCmd : public ToolCommand
{
    uint32_t signum;                     //!< Signal number to send.
};
...
struct SendSignalToProcessAckCmd : public ToolCommand
{
};
